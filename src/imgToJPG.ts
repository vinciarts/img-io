...
const compressImage = async (file: File): Promise<ImgProcessorResult> => {
	try {
		const options = {
			maxSizeMB: 0.5,
...
// final
export const imgToJPG = async (file: File): Promise<ImgProcessorResult> => {
	try {
		if (file.type === 'image/heic' || file.type === 'image/heif') {
			return await compressImage(await convertHEICtoJPEG(file));
		} else {
